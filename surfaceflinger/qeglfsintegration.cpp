...
{
    QEglFSWindow *w = new QEglFSWindow(mSfc, window);
    w->create();
...
{
    return new QEglFSContext(mSfc, static_cast<QEglFSPageFlipper *>(mScreen->pageFlipper()), mSfc->surfaceFormatFor(context->format()), context->shareHandle(), mDisplay);
}

QPlatformOffscreenSurface *QEglFSIntegration::createPlatformOffscreenSurface(QOffscreenSurface *surface) const
{
    QEglFSScreen *screen = static_cast<QEglFSScreen *>(surface->screen()->handle());
    return new QEGLPbuffer(screen->display(), mSfc->surfaceFormatFor(surface->requestedFormat()), surface);
}
