...
{
    return QRect(QPoint(0, 0), m_sfc->screenSize());
}

int QEglFSScreen::depth() const
{
    return m_sfc->screenDepth();
}

QImage::Format QEglFSScreen::format() const
{
    switch (m_sfc->screenDepth()) {
        case 16:
...
{
    return m_sfc->physicalScreenSize();
}

QDpi QEglFSScreen::logicalDpi() const
{
    QSizeF ps = m_sfc->physicalScreenSize();
    QSize s = m_sfc->screenSize();
